...
function isPromotionActive(promotion) {
    ...
    if (promotion.allowedDays?.length) {
        ...
        if (!promotion.allowedDays.includes(currentDay))
            return false;
    }
    return true;
}
function targetMatchesPromotion(promotion, targetType, targetId) {
    const collection = targetType === 'PACKAGE' ? promotion.packages : promotion.services;
    return collection?.some((item) => (targetType === 'PACKAGE' ? item.packageId : item.serviceId) === targetId);
}
export class PrismaCatalogRepository {
    syncPackagePrice(packageId) {
        return prisma.catalogPackage.findUnique({ where: { id: packageId }, include: { items: true } }).then((record) => {
            if (!record)
                return null;
    ...
    async listPackages() {
        ...
        return records.map((record) => packageFinancials(record));
    }
    ...
    async upsertPackageItem(packageId, data) {
        if (data.id) {
    ...
    async updateInventoryCost(id, newCost) {
    ...
    async upsertServiceComponent(serviceId, data) {
        const payload = {
            inventoryItemId: data.inventoryItemId ? String(data.inventoryItemId) : null,
            name: String(data.name),
            unit: data.unit ? String(data.unit) : null,
    ...
    async deleteServiceComponent(componentId) {
    ...
    async updatePromotion(id, data) {
        ...
        await prisma.promotion.update({
            ...
            data: {
                ...
                allowedDays: Array.isArray(data.allowedDays) ? data.allowedDays : undefined,
    ...
    async togglePromotion(id, active) {
    ...
    async setPromotionTargets(id, data) {
    ...
    async listPromotionPreview() {
        ...
        const packagePreview = packages.map((record) => {
        ...
        const servicePreview = services.map((record) => {
            const promotion = promotions.find((candidate) => isPromotionActive(candidate) && targetMatchesPromotion(candidate, 'SERVICE', record.id));
            const discountPercent = promotion ? toNumber(promotion.discountPercent) : 0;
