...
export class CatalogPackagesHandler {
    repository;
    ...
    handle(input) {
        ...
        switch (data.action) {
            ...
            case 'detail': {
                if (!data.id)
                    throw new HttpError(400, 'Se requiere el identificador del paquete.');
            ...
            case 'upsertItem': {
                if (!data.packageId || !data.item)
                    throw new HttpError(400, 'Se requiere paquete e ítem.');
                return this.repository.upsertPackageItem(data.packageId, data.item);
            }
            case 'deleteItem': {
                if (!data.itemId)
                    throw new HttpError(400, 'Se requiere el ítem a eliminar.');
