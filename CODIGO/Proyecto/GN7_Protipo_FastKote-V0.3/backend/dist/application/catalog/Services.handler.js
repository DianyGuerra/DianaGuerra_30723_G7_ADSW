...
export class ServicesHandler {
    ...
    handle(input) {
        ...
        switch (data.action) {
            ...
            case 'update': {
                if (!data.id)
                    throw new HttpError(400, 'Se requiere el identificador del servicio o producto.');
            ...
            case 'upsertComponent': {
                if (!data.id || !data.component)
                    throw new HttpError(400, 'Se requiere el servicio y el componente.');
                return this.repository.upsertServiceComponent(data.id, data.component);
            }
            case 'deleteComponent': {
                if (!data.componentId)
                    throw new HttpError(400, 'Se requiere el componente a eliminar.');
